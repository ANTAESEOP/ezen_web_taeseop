...
public class Ex1_배열 { // class start
	...
	System.out.println( "배열[2] : " + scores[2] ); // 배열내 1번 인덱스 호출 -> 87
	System.out.println("배열 : " + scores);			// 배열명 주소[번지] 호출
	// 3. 배열 반복문 활용
	int sum = 0;
	for( int i = 0; i<scores.length ; i++) { // i는 0부터 배열의길이 전까지 1씩 증가
		sum += scores[i];
	} // for end
	...
	for( int temp : 배열) { 
		// for( 반복변수 : 배열명 )
		// 배열내 첫번째 인덱스부터 마지막 인덱스까지 반복변수에 하나씩 대입
