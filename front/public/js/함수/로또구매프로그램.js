...
// * 변수 선언 
// 변수는 지역변수 특징 { } 안으로 들어갈 수 있다. { } 밖으로 못나감
let select_numbers = [ ] // 내가 선택한 번호 리스트 // 여러게 함수에서 호출(사용 될 예정)
let lotto_numbers = [ ]  // 컴퓨터 난수 번호 리스트 

// 1. 버튼 만들기 함수
function btn_print(){
	let btnlist = '' // 1~45의 변수를 저장하는 변수
	for( let i = 1 ; i<=45 ; i++ ){
			// i는 1부터 45까지 1씩증가 반복
	...
	// 현재문서의 btnbox 라는 id 찾는 태그 호출해서 html에 변수 대입
...
function lotto_number(){
	...
	while(true){	// 무한루프 [ 추첨번호가 6개 되면 종료 ]
		...
		let rand = Math.floor( (Math.random()*45)+1)		// Math.random
