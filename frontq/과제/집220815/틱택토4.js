/*
		1. 9개 버튼 생성 [ 게임판 변수에 모든 버튼을 저장한다. ]
...
function 게임판그리기(){
		...
		승리 ='' // 승리 변수를 공백으로 초기화
		게임상황 = [null,null,null,null,null,null,null,null,null] // 배열에 9개 null 초기화
		let 게임판 = '' // 빈 변수 선언
...
function 버튼선택 ( i ){
		...
		document.getElementById( i ).innerHTML = 'O' // 1. 선택한 번호의 버튼 모영 변경
		...
		while( ture ){ //while(ture) : 무한루프 [ 종료조건 : 랜덤위치에 알을 두었을때 반복종료 ]
	...
	function 중복검사 ( i ){
			if( 게임상황[i] != null ){ // 배열은 모두 null로 초기화 했기 때문에 null 아니면 'O' 또는 'X'가 존재하는 의미 [ 알이 있다. ]
					return ture // 알이 있다. null 이 아니면
			}else{		
	...
	function 게임결과(){
			...
			for( let i = 0 ; i<=2 ; i++ ){
					...
					if( 게임상황[i] !=null && 게임상황[i] == 게임상황[i+3] && 게임상황 [1+3] == 게임상황[i+6])
							승리 = 게임상황[i] // 승리 변수에 i번째 알 저장
							return ture // 가로로 승리가 나왔을때 경우 함수 종료
					}
			}	
