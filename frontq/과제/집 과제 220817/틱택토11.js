...
// 2. 버튼을 눌렀을때 이벤트함수 [ 매개변수 : i = 선택한 버튼 번호 ]
function 버튼선택 ( i ){
	 
		if( 중복검사(i) ){ // 3. 함수 호출해서 해당 위치에 알이 있으면 함수 종료 (return)
		...
		// 2. 난수 발생 [Math.floor(실수) : 손수점 버림 / math.random() : 0~1 사이의 실수 난수 발생 ]
		while( true ){// while(true) : 무한루프[ 종료조건 : 랜덤위치에 알을 두었을때 반복 종료 ]
...
function 중복검사 ( i ){
		if( 게임상황[i] !=null ){
				return true // 알이 있다. null이 아니면
		}else{
				return flase // 알이 없다 . null이면
		} // if end
} // f end
// 4. 현재 상황에 게임곃과 판단하는 함수
function 게임결과(){
		...
		for( let i = 0 ; i<=6 ; i+=3){
				...
				if( 게임상황[i] !=null && 게임상황[i] == 게임상황 [i+1] && 게임상황 [i+1] == 게임상황[i+2]){
						// && 이면서 면서 이고 모두 그리고
						// i 가 0일때 0 == 1 == 2 	// i가 3일때 3 == 4 == 5 // i가 6일때 6 == 7 == 8
