...
// 2. 버튼을 눌렀을대 이벤트 함수 [ 매개변수 : i = 선택한 버튼 번호 ]
function 버튼선택 ( i ){
		...
		if( 게임곃과 ( ) ) { // 게임결과 함수에서 ture가 리턴되면
				alert('게임종료 승리 : ' + 승리 ) 
				return // 함수 강제 종료
		} // if end
		
		// 2. 난수 발생 [ Math.floor(실수) : 소숫점 버림 / Math.random() : 0 ~ 1 사이의 실수 난수 발생 ]
...
function 게임결과(){
		...
		for( let i = 0 ; i<=6 ; i+=3){
				...
				if( 게임상황[i] !=null && 게임상황[i] == 게임상황[i+1] && 게임상황[i+1] == 게임상황[i+2] ){
						// && 이면서 면서 이고 모두 그리고
						// i가 0일대 0 == 1 == 2 		//i가 3일때 3 == 4 == 5  // i가 6일때 6 == 7 == 8
