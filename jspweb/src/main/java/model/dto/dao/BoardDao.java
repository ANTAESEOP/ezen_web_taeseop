...
public class BoardDao extends Dao {
	...
	private static BoardDao bdao = new BoardDao();			// 싱글톤
	...
	// 9-2 . 대댓글 작성
		public boolean rrwrite( String rcontent , int mno , int bno , int rindex ) {
			...
			try {
				ps = con.prepareStatement(sql);
				ps.setString(1, rcontent); ps.setInt(2, mno); ps.setInt(3, bno); ps.setInt(4 , rindex);
				ps.executeUpdate();return true;
			}catch (Exception e) {System.out.println(e);} return false;
		}
	...
	public JSONArray getapi() {
		JSONArray array = new JSONArray();
		String sql = "select * from 아파트실거래가 where 시군구 like '%경기도 군포%'";
		try {
			ps = con.prepareStatement(sql);
			rs = ps.executeQuery();
			while(rs.next() ){
