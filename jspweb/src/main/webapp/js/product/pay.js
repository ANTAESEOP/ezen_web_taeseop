...
function requestPay(  ) {
	...
	IMP.request_pay({
		...
		name: checkplist[0].pname + "외 " + checkplist.length, // 잡아구니에 있는 것
		amount: "",                     // 숫자타입
		buyer_email:"",     
		buyer_name: "",    
		buyer_tel: "",    
...
function setorder(){
	...
	$.ajax({
		...
		data :{ "data" : JSON.stringify( checkplist ), // JSON.stringify 문자열 타입으로 객체를 보냄
		 		"oinfo" : JSON.stringify( oinfo ) },
		type : "post",
		success : (re) => {
			if( re == 'true'){ 
				alert( ' 주문 완료 '); 
				localStorage.setItem('checkplist') // 저장소 제거
